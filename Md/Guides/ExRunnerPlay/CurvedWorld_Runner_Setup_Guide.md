...
2. **장애물 시스템**: `UExObstacleDefinition` 데이터 에셋 활용 (완료)
3. **입력 처리**: 점프, 슬라이드 (완료 - 예측 회전 보정 포함)
4. **레인 시스템**: Y축 이동 (완료)
...
곡선 구간에서 점프 시 관성으로 인해 레인을 이탈하는 것을 방지하기 위해 **예측 회전(Predictive Rotation)** 기능이 포함되어 있습니다.

### 조율 방법:
1. **커브 꺾임 강도**: `ExGameModeDataSet` 데이터 에셋의 `JumpYawPredictionWeight` 값을 수정합니다.
   - 1.0: 예상 곡률만큼 100% 회전
   - 0.5: 반만 회전 (더 넓게 회전)
   - 1.5: 더 많이 회전 (안쪽으로 파고듦)

2. **공중 제어력**: 캐릭터 블루프린트의 **Mover 컴포넌트 설정**에서 `AirControl` 관련 수치를 **0.8 ~ 1.0**으로 높게 설정해야 공중에서 방향타가 먹힙니다.
